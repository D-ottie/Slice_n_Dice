...
public class Targets : MonoBehaviour
{
    ...
    public void DestroyTarget()
    {
        if(!FindObjectOfType<GameManager>().gameOver)
        {
            ...
            for (int i = 0; i< explsosionParticle.Length ; i++)
            {
                Instantiate(explsosionParticle[i], transform.position, explsosionParticle[i].transform.rotation);
            }

            if (gameObject.CompareTag("Bad"))
            {
                FindObjectOfType<GameManager>().StopAllMonsters();
